...
//		this.init = init;
	...
	private RedisTool(String redisPoolName,boolean init){
	...
	public static RedisTool getInstance(boolean init){
		if(!init) {
			...
			synchronized (RedisTool.class) {
				if (defaultRedisTool != null)
					return defaultRedisTool;
				return defaultRedisTool = new RedisTool(init);
			}
		}
		else{
			...
			synchronized (RedisTool.class) {
				if (initDefaultRedisTool != null)
					return initDefaultRedisTool;
				return initDefaultRedisTool = new RedisTool(init);
			}
		}
	}
	...
	public static RedisTool getInstance(String redisPoolName){
		return getInstance( redisPoolName,true);
	}
	/**
	 *
	 * @param redisPoolName
	 * @param init 如果redis没有初始化，true 进行初始化 false不初始化
	 * @return
	 */
	public static RedisTool getInstance(String redisPoolName,boolean init){
		if(!init) {
			...
			synchronized (RedisTool.class) {
				...
				if (redisTool != null)
					return redisTool;
				redisTool = new RedisTool(redisPoolName,init);
				redisToolMap.put(redisPoolName, redisTool);
			}
			return redisTool;
		}
		else{
			RedisTool redisTool = initRedisToolMap.get(redisPoolName);
			if (redisTool != null)
				return redisTool;
			synchronized (RedisTool.class) {
				redisTool = initRedisToolMap.get(redisPoolName);
				if (redisTool != null)
					return redisTool;
				redisTool = new RedisTool(redisPoolName,init);
				initRedisToolMap.put(redisPoolName, redisTool);
			}
			return redisTool;
		}
	}
...
//	   */
    ...
    public boolean tryLockWithRetry(String lockKey, String lockValue, int expireTime, int retryTimes, long retryInterval)  {
        ...
        {
            redisHelper = RedisFactory.getRedisHelper(redisPoolName,init);
            return redisHelper.tryLockWithRetry(  lockKey,   lockValue,   expireTime,   retryTimes,   retryInterval) ;
        }
        finally
        {
            if(redisHelper != null)
